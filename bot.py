...
class AddStationModal(discord.ui.Modal, title="API 配置管理器"):
    """填 配置名称/Custom API URL/密钥/(可选)首选模型；提交后自动拉「该 url+key 可用的模型」
    并完成连通测试，选模型才保存。填了首选模型则直接用它测试，不用再选一次。"""
    f_label = discord.ui.TextInput(
        label="配置名称（自取，方便你自己区分）", required=False,
        placeholder="例如：OpenAI GPT-4", max_length=20)
    f_base = discord.ui.TextInput(
        label="Custom API URL（OpenAI 兼容）",
        placeholder="例如：https://api.openai.com/v1")
    f_key = discord.ui.TextInput(
        label="Custom API 密钥（只存内存、永不回显明文）", placeholder="sk-...")
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not base or not key:
            ...
            return
        if model:
            # 填了首选模型：直接用它做一次连通测试，过了就保存，不用再走选模型流程
            await interaction.response.send_message(
                f"⏳ 正在用模型「{model}」测试连接…", ephemeral=True)
            profile = {"name": label, "base_url": base, "api_key": key, "model": model}
            good, why = await llm.health_check_profile(profile)
            if not good:
                await interaction.followup.send(
                    f"🔴 测试失败：{why}\n（没有保存，请检查 URL/密钥/模型名后重试）", ephemeral=True)
                return
            ok, info = userapi.add_station(interaction.user.id, label, base, key, model)
            if ok:
                await database.upsert_station(interaction.user.id, info, base, key, model)
            msg = (f"🟢 测试通过，已保存配置「{info}」（模型 {model}）。"
                   if ok else f"❌ {info}")
            await interaction.followup.send(msg, ephemeral=True)
            return
        await interaction.response.send_message(
            "⏳ 正在测试连接并获取模型列表…", ephemeral=True)
        ...
        if ok and isinstance(result, list) and result:
            ...
            await interaction.followup.send(
                f"🟢 连接成功，获取到 {len(result)} 个模型。选一个要用的（或手动填）：",
                view=view, ephemeral=True)
        elif ok:
...
class EditStationModal(discord.ui.Modal):
    ...
    def __init__(self, hub: "ApiHubView", old_label: str):
        super().__init__(title=f"编辑配置「{old_label}」"[:45])
        self._hub = hub
        self._old = old_label
        st = userapi.get_station(hub.uid, old_label)
        self.f_label = discord.ui.TextInput(
            label="配置名称", default=(st.label if st else old_label),
            required=False, max_length=20)
        self.f_base = discord.ui.TextInput(
            label="Custom API URL", default=(st.base_url if st else ""))
        self.f_key = discord.ui.TextInput(
            label="Custom API 密钥（留空=保持原密钥不变）", required=False,
            placeholder="不改就留空")
        self.f_model = discord.ui.TextInput(
            label="首选模型", default=(st.model if st else ""))
